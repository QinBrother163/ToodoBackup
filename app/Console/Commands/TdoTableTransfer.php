...
class TdoTableTransfer extends Command
{
    protected $signature = 'tdo:trans {table} {field=null} {primary=null} {now=null}';
    protected $description = 'trans a table from srcDB to dstDB refer by field';
    ...
    public function handle()
    {
        ...
        if (empty($field) || $field == 'null') {
            \Log::info("-e tdo:trans $table field empty!");
            return;
        }
    ...
    protected function check($table, $primary, $field, $now)
    {
        ...
        if ($this->trans($table, $primary, $field, $begin, $end)) {
            ...
            $log->save();
            \Log::info("-e tdo:trans $table $field $date $hour");
        }
    }

    /**
     * @param $dstDb \Illuminate\Database\Connection
     * @param $srcDb \Illuminate\Database\Connection
     * @param $table string
     * @param $primary null|string
     * @return bool
     */
...
//        \Log::info('trans 1 begin');
        $dstDb = DB::connection('mysql');
        $srcDb = DB::connection('mysql.tdsrv');
